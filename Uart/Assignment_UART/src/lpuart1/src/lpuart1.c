...
*******************************************************************************/
/**
  * @brief                   calculate baud rate of UART protocol
  * uint32_t clock           clock of UART module
  * uint32_t baud            baud rate to calculate
  * uint16_t *OSR            pointer to OSR
  * uint16_t *SBR            pointer to SBR
  * @return value            None
  */
void cacul_baudrate(uint32_t clock, uint32_t baud, uint16_t *OSR, uint16_t *SBR);

/**
  * @brief                   enable IRQ handler
...
/**
  * @brief                   send charter through UART protocol
  * char character           character that been sent
  * @return value            None
  */
...
void init_uart(const uint32_t _baud_rate)
{
	...
	IP_LPUART1->BAUD |= LPUART_BAUD_OSR(OSR);//15

	IP_LPUART1->BAUD &= ~LPUART_BAUD_SBR_MASK;
	IP_LPUART1->BAUD |= LPUART_BAUD_SBR(SBR);//26
